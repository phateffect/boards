...
OV3660 在本仓库被建模为“传感器/摄像头模组事实源”，不是某个具有统一连接器的商品模组。不同载板会使用不同 FPC 或直接布线，因此不存在可跨板复用的 Pin 1 排布。

```text
典型光学方向（概念图，不代表某一 FPC 引脚编号）
        ...
        │  镜头组  │
        ├─────────┤
        │ OV3660  │── D0..D7 / PCLK / VSYNC / HREF
        │ 传感器   │── XCLK / RESET / PWDN
        │         │── SCCB SIOC / SIOD
        └────┬────┘
             └──── 具体 FPC/载板连接器：以载板页面为准
```

当前使用实例 Timer Camera F 为板内直连，具体 DVP GPIO 和整机接口方向见 [Timer Camera F 的物理排布](../../boards/m5stack-timer-cameraf/README.md#引脚物理排布)。在没有某一具体 OV3660 FPC 型号/图纸时，不能为本页虚构 24-pin 或其他排针顺序。
